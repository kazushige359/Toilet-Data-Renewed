...
//            
        ...
        func dataQuery(queryKey: String){
            ...
            toiletsRef.child(queryKey).observe(FIRDataEventType.value, with: { snapshot in
                ...
                toilet.urlTwo = (snapshotValue?["urlTwo"] as? String!)!
                toilet.urlThree = (snapshotValue?["urlThree"] as? String)!
                toilet.address = (snapshotValue?["address"] as? String)!
                toilet.howtoaccess = (snapshotValue?["howtoaccess"] as? String)!
                toilet.addedBy = (snapshotValue?["addedBy"] as? String)!
                toilet.editedBy = (snapshotValue?["editedBy"] as? String)!
                toilet.openinghours = (snapshotValue?["openAndCloseHours"] as? String)!
                ...
                toilet.loc = CLLocation(latitude: toilet.latitude, longitude: toilet.longitude)

                let distance = toilet.loc.distance(from: self.search.centerSearchLocation)
