...
class YouWentTableViewController: UITableViewController, CLLocationManagerDelegate, MKMapViewDelegate
{
    ...
    func firebaseQuery(){
        let firebaseRef = FIRDatabase.database().reference()
        firebaseRef.child("users").child(FIRAuth.auth()!.currentUser!.uid).child("youwent").observe(FIRDataEventType.childAdded, with: {(snapshot) in
            print("First Snap!!")
            print(snapshot)
...
//        }}
    ...
    func multipleDeleteExecution(){
        for item in deleteArray{
            firebaseRef.child("users").child(FIRAuth.auth()!.currentUser!.uid).child("youwent").child(item).removeValue { (error, ref) in
                if error != nil{
                    print("Failed to delete a cell",error!)
                    return
                }
    ...
    func deleteAll(){
        ...
        firebaseRef.child("users").child(FIRAuth.auth()!.currentUser!.uid).child("youwent").removeValue { (error, ref) in
            if error != nil{
                print("Failed to delete a cell",error!)
                return
            }
